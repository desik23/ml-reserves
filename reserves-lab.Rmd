...
2. What is the highest cost value?
  # The highest cost value is 47.23834. 
3. Is there a spatial pattern in the planning unit cost values (hint: use `plot` to make a map)?
  #Yes, the planning unit cost values are lower in the east when compared to the west. There also seems to be very high cost values in the center of the northern border. 
```
...
```{r, eval = FALSE}
# plot an interactive map of the 13th vegetation class
mapview(veg_data[[13]])
```
...
# calculate the sum of all the cell values in the 12th layer
cellStats(veg_data[[12]], "sum")
...
```{r}
# calculate proportion of cells contain the 12th vegetation class
cellStats(veg_data[[12]], "sum")/ ncell(veg_data)
```

```{r}
# calculate vegetation class that is the most abundant
which.max(cellStats(veg_data, "sum", na.rm = TRUE))
```
...
# The 13th vegetation class is more prevalent in the North-eastern region.
2. What proportion of cells contain the 12th vegetation class?
# 1.53%  of the cells contain the 12th vegetation class
3. Which vegetation class is the most abundant (i.e. present in the greatest number of cells)?
# The 12th vegetation class is the most abundant class. 
```
...
```{r}
# find the median abundance of the feature
median(abundance_data$absolute_abundance_km2)
# name of the feature with smallest abundance
abundance_data$feature[which.min(abundance_data$absolute_abundance_km2)]
# features have a total abundance greater than 100 km^2
...
# The median abundance is 19.12 km^2. 
2. What is the name of the feature with smallest abundance?
# The name of the feature with smallest abundance is vegetation 3. 
